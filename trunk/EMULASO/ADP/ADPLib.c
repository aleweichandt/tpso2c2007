...
void InformarLista( tListaPCB Lista )
{
	...
	else if ( Lista == ADP.m_LPN && ADP.m_LPN )
	{
		strcpy(szInfo, "LPN: ");
		y = 0;
	}
	...
	ADP_printToWin( ADP.m_pwInfo, szInfo,y );
	Log_printf( log_debug, szInfo );
}
...
void ADP_Dispatcher(int n)
{
	...
	if ( ADP_SeCumplioQ() )
	{
		...
		ADP_printToWin( ADP.m_pwLogger, "<< Entra al Dispatcher >>",1 );
		...
		Log_printf( log_debug, "LPN" );
		InformarLista( ADP.m_LPN );
		Log_printf( log_debug, "LPB" );
		InformarLista( ADP.m_LPB );
		Log_printf( log_debug, "LPL" );
		InformarLista( ADP.m_LPL );
		...
		ADP_printToWin( ADP.m_pwLogger, "<< Sale del Dispatcher >>",1 );
		
		
/*		sprintf( szInfo, "Cant PCBs a aceptar %d, Mem Disp %d", ADP.m_nCantDisp, ADP.m_nMemDisp );
		ADP_printToWin( ADP.m_pwInfo, szInfo,4 );*/
	}
	...
	fCarga = ADP_CalcularCargaPromReal();
	sprintf( szInfo, "Cant PCBs a aceptar %d, Mem Disp %d, Carga %f", ADP.m_nCantDisp, ADP.m_nMemDisp, fCarga );
	ADP_printToWin( ADP.m_pwInfo, szInfo,4 );
...
int 	ADP_InformarSuspencion() 
{
	...
	while( Lista )
	{
		...
		if ( pPCB->Q <= 0 )
		{
			...
			ADP_printfToWin( ADP.m_pwLogger, 1,"envio Suspension al pcb %ld", pPCB->id  );
...
void ADP_InformarReanudacion()
{
	...
	while( Lista )
	{
		...
		ADP_printfToWin( ADP.m_pwLogger,1,"Envio Start al PCB: %ld por fd = %d", 
						pPCB->id, pPCB->pSocket->descriptor );
...
{/*Asumo que se refiere a cualquier PCB, este en el estado que este*/
	...
	pcb1 = ADP_BuscarPCBMayorTRestante( ADP.m_LPN, &nMayor, pcb1 );
	...
	if ( pcb1 )
	{
		...
		if ( pcb1->pSocket )
		{
			...
			ADP_printfToWin( ADP.m_pwLogger,1,"Decido sacar al pcb %ld", pcb1->id );
...
tunPCB* ADP_BuscarPCBMayorTRestante( tListaPCB Lista, int *nMayor, tunPCB* pcb1 )
{
	...
	while( Lista )
	{
		...
		else
		{
			...
			ADP_printfToWin( ADP.m_pwLogger,1, "No se pudo leer el %s!!", szArchivo );
			nTimeRemaining = 1000;
		}
 ...
 * te va a romper a cada segundo. ;)*/
{
	...
	else if ( senial == SIGKILL || senial == 9 )
	{
		Log_log( log_warning, "Recibo senial SIGKILL");
		/*ADP_Salir();*/
	}
	else if ( senial == SIGUSR1 )
	{
		Log_log( log_warning, "Recibo senial SIGUSR1");
		ADP_printToWin( ADP.m_pwLogger, "Recibo senial SIGUSR1",1);
	...
	{	
		...
		ADP_printToWin( ADP.m_pwLogger, "Recibo senial SIGCHILD",1);
...
int ADP_Init( )
{
	...
	do
	{
		...
		if ( ADP_ConectarConACR() == ERROR )
		{
			...
			ADP_printToWin( ADP.m_pwLogger, "No se pudo establecer conexion con el ACR!!!",1 );
			return ERROR;
		}
		...
		/*signal(SIGKILL, ADP_ProcesarSeniales);*/
...
int ADP_LeerConfigOnline()
{
	...
	{		
		...
		if ( ADP.m_Q != Q )
		{
			...
			ADP_printfToWin( ADP.m_pwLogger,1, "Se cambia el Q de %d a %d", ADP.m_Q, Q );
			ADP.m_Q = Q;
		}
...
void ADP_ConfirmarConexion( tSocket* sockIn )
{
	...
	{/*Si el ACR me responde pong la conexion queda establecida!*/
		...
		ADP_printToWin( ADP.m_pwLogger, "Conexion establecida con el ACR!",1 );
 ...
 * se identifica el proceso remitente, en base a este se le asigna un handler especifico*/
{
	...
	{/*Bienvenido PCB!*/
		...
		if ( (pcb = lpcb_BuscarPCBxid( &ADP.m_LPN, lpcb_id )) ) /*Busco el pcb y actualizo el socket*/
		{
			Log_printf( log_debug, "ya tengo el pcb %ld en la cola de nuevos", pcb->id );
			pcb->pSocket = sockIn;
						
			if ( pcb->Registro[0] == BLOQUEADO )
			{
				ADP_printfToWin( ADP.m_pwLogger,1, "Estado del pcb %ld = %d, lo agrego en Listos", pcb->id,  (int) pcb->Registro[0] );
				lpcb_PasarDeLista( &ADP.m_LPN, &ADP.m_LPB, pcb->id );
			}
			else
			{
				bListo = 1;
				ADP_printfToWin( ADP.m_pwLogger,1, "Estado del pcb %ld = %d, lo agrego en Listos", pcb->id,  (int) pcb->Registro[0] );
				lpcb_PasarDeLista( &ADP.m_LPN, &ADP.m_LPL, pcb->id );
			}
		}
...
/**/
{
	...
	{/*Me pide la performance*/
		...
		ADP_printToWin( ADP.m_pwLogger, "el ACR me manda un GetPerformance",1 );
	...
	else if( IS_PAQ_GET_PCBS_STATES( paq ) )
	{
		...
		ADP_printToWin( ADP.m_pwLogger, "el ACR me manda un GetPCBsStates",1 );
	...
	else if( IS_PAQ_SOL_CONCEDIDO(paq) )
	{
		...
		ADP_printToWin( ADP.m_pwLogger, "recibo PAQ_SOL_CONCEDIDO",1);
...
/**********************************************************/
void ADP_AtenderPCBNuevo ( tSocket *sockIn )
/**/
{
	int 			len; 
	char			buffer [ PAQUETE_MAX_TAM ];
	...
	if ( ERROR == len || !len)
	{
		Log_logLastError("Se cierra socket al atender PCB Nuevo");
		conexiones_CerrarSocket(ADP.m_ListaSockets,sockIn,&ADP.m_ultimoSocket);		
...
/**/
{
	...
	{/*Me llega el paquete migrar, cambio el handshake para recibir el archivo del pcb data*/
		...
		ADP_printfToWin( ADP.m_pwLogger,1, "Recibiendo al pcb %ld por migracion", lpcb_id );
	...
	{/*me llega el print, se lo reenvio al ACR*/
		...
		if ( lpcb_id == 0 || (lpcb_id > 0 && !lpcb_BuscarPCBxid( &ADP.m_LPN, lpcb_id)) )
		{/*Si no viene con el pcb_id O viene y no esta en la cola de Nuevos, entonces mando el print
		Esto es porque cuando migra, el ACR le corta la conexion y llega un print con id por muerte inesperada,
		entonces no quiero que cuando este print llegue al ACR le devuelva los recursos.*/
			Log_log( log_debug, "Mando PRINT al ACR" );
			ADP_printToWin( ADP.m_pwLogger, "Mando PRINT al ACR",1 );
		...
		if ( lpcb_id < 0 )
		{
			mig = 1;
			lpcb_id *= (-1);
			Log_printf(log_debug,"Migra el PCB %ld", lpcb_id );
			ADP_printfToWin( ADP.m_pwLogger,1,"Migra el PCB %ld", lpcb_id );
		}
		
		if( lpcb_id  > 0 )
		{/*Entonces es el print final*/
			Log_printf(log_debug,paq->msg );
			ADP_printfToWin( ADP.m_pwLogger,1,paq->msg );
	...
	{/*me llega el dev, se lo reenvio al ACR*/
		...
		ADP_printToWin( ADP.m_pwLogger, "Mando DEV al ACR",1 );
	...
	else if( IS_PAQ_SOL( paq ) )
	{
		...
		ADP_printToWin( ADP.m_pwLogger, "Se recibe un paq Sol",1 );
...
void ADP_EliminarDeLista( long id )
{
	...
	if ( lpcb_BuscarPCBxid( &ADP.m_LPL, id )  )
	{
		...
		Log_printf(log_debug,"Elimino el PCB %ld de la lista LPL", id);
		ADP.m_nCantDisp++;
		return;
	}
	else if ( (pcb = lpcb_BuscarPCBxid( &ADP.m_LPE, id )) )
	{
		/*Tengo que devolver la memoria que estaba usando!!!*/
		ADP.m_nMemDisp += pcb->MemoriaRequerida;
				
		lpcb_EliminarDeLista( &ADP.m_LPE, id );
		Log_printf(log_debug,"Elimino el PCB %ld de la lista LPE", id);
		ADP.m_nCantDisp++;
		return;
	}
	else if ( lpcb_BuscarPCBxid( &ADP.m_LPB, id )  )
	{
		lpcb_EliminarDeLista( &ADP.m_LPB, id );
		Log_printf(log_debug,"Elimino el PCB %ld de la lista LPB", id);
		ADP.m_nCantDisp++;
		return;
	}
	else if ( lpcb_BuscarPCBxid( &ADP.m_LPN, id )  )
	{
		lpcb_EliminarDeLista( &ADP.m_LPN, id );
		Log_printf(log_debug,"Elimino el PCB %ld de la lista LPN", id);
		ADP.m_nCantDisp++;
		return;
	}
...
void ADP_RecibirArchivo( tSocket *sockIn )
{
	...
	{/*Llega el paq archivo -> persisto el contenido*/
		...
		ADP_printToWin( ADP.m_pwLogger, "Llega un PAQ_ARCHIVO",1 );
	...
	else if ( IS_PAQ_FIN_MIGRAR( paq ) )
	{
		...
		ADP_printToWin( ADP.m_pwLogger, "llega un PAQ_FIN_MIGRAR",1 );
...
{/*Cerrar el socket correspondiente y tomar la accion correspondiente*/
	...
	if ( (pPCB = ADP_BuscarPCBxSocket( sockIn )) )
...
float	ADP_CalcularCargaPromReal()
{
	...
	if ((archivo = fopen("/proc/loadavg", "r")) == NULL) 
	{
		...
		ADP_printfToWin( ADP.m_pwLogger,1, "Error abriendo archivo del load average" );
		return 0;
	}
...
{/*Si le seteo el socket este no me sirve, porque se cortara la conexion*/
	...
	{
		pSock->callback = &ADP_AtenderPCBNuevo;
		pcb->Registro[0] = pcb_state;

		ADP.m_nCantDisp--;
		Log_printf( log_debug, "Estado del pcb %ld = %d", pcb->id,  (int) pcb_state );
		ADP_printfToWin( ADP.m_pwLogger,1, "Estado del pcb %ld = %d", pcb->id,  (int) pcb_state );
		return lpcb_AgregarALista( &ADP.m_LPN, pcb );
...
/*		
		if ( pcb_state == BLOQUEADO )
		{
			...
			ADP_printfToWin( ADP.m_pwLogger,1, "Estado del pcb %ld = %d, Bloqueado", pcb->id,  (int) pcb_state );
		...
		else
		{
			...
			ADP_printfToWin( ADP.m_pwLogger,1, "Estado del pcb %ld = %d, lo agrego en Listos", pcb->id,  (int) pcb_state );
...
void ADP_LiberarRecursos(int pid){
	...
	for(i=0; i<4; i++){
				...
				if(pcb->id == pid){
					...
					switch (i){
						...
						case 2:{
							...
							break;}
						case 3:{
							lpcb_EliminarDeLista( &ADP.m_LPN, pcb->id );
							SeElimino = 1;
							break;}
					}
				}
		...
		if ( SeElimino )
		{
			/*menos mal que me di cuenta, incremento los pcbs que admito*/
			ADP.m_nCantDisp++;
			break;
		}
	}
